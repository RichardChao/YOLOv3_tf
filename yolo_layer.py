...
class yolo_head:
    ...
    def __init__(self, istraining, trainable=True):
        self.istraining = istraining
        self.trainable = trainable
    def conv_layer(self, bottom, size, stride, in_channels, out_channels, use_bn, name, trainable=True):
        with tf.variable_scope(name):
            conv = tf.layers.conv2d(bottom, out_channels, size, stride, padding="SAME",
                                    use_bias=not use_bn, activation=None, trainable=trainable)
            if use_bn:
    ...
    def build(self, feat_ex, res18, res10):
        ...
        self.conv52 = self.conv_layer(feat_ex, 1, 1, 1024, 512, True, 'conv_head_52', self.trainable)  		# 13x512
        self.conv53 = self.conv_layer(self.conv52, 3, 1, 512, 1024, True, 'conv_head_53', self.trainable)   # 13x1024
        self.conv54 = self.conv_layer(self.conv53, 1, 1, 1024, 512, True, 'conv_head_54', self.trainable)   # 13x512
        self.conv55 = self.conv_layer(self.conv54, 3, 1, 512, 1024, True, 'conv_head_55', self.trainable)   # 13x1024
        self.conv56 = self.conv_layer(self.conv55, 1, 1, 1024, 512, True, 'conv_head_56', self.trainable)   # 13x512
        self.conv57 = self.conv_layer(self.conv56, 3, 1, 512, 1024, True, 'conv_head_57', self.trainable)   # 13x1024
        self.conv58 = self.conv_layer(self.conv57, 1, 1, 1024, final_filter_num, False, 'conv_head_58')   # 13x125
        # follow yolo layer mask = 6,7,8
        self.conv59 = self.conv_layer(self.conv56, 1, 1, 512, 256, True, 'conv_head_59', self.trainable)    # 13x256
        ...
        self.conv60 = self.conv_layer(self.route0, 1, 1, 768, 256, True, 'conv_head_60', self.trainable)    # 26x256
        self.conv61 = self.conv_layer(self.conv60, 3, 1, 256, 512, True, 'conv_head_61', self.trainable)    # 26x512
        self.conv62 = self.conv_layer(self.conv61, 1, 1, 512, 256, True, 'conv_head_62', self.trainable)    # 26x256
        self.conv63 = self.conv_layer(self.conv62, 3, 1, 256, 512, True, 'conv_head_63', self.trainable)    # 26x512
        self.conv64 = self.conv_layer(self.conv63, 1, 1, 512, 256, True, 'conv_head_64', self.trainable)    # 26x256
        self.conv65 = self.conv_layer(self.conv64, 3, 1, 256, 512, True, 'conv_head_65', self.trainable)    # 26x512
        self.conv66 = self.conv_layer(self.conv65, 1, 1, 512, final_filter_num, False, 'conv_head_66')    # 26x125
        # follow yolo layer mask = 3,4,5
        self.conv67 = self.conv_layer(self.conv64, 1, 1, 256, 128, True, 'conv_head_67', self.trainable)    # 26x128
        ...
        self.conv68 = self.conv_layer(self.route1, 1, 1, 384, 128, True, 'conv_head_68', self.trainable)    # 52x128
        self.conv69 = self.conv_layer(self.conv68, 3, 1, 128, 256, True, 'conv_head_69', self.trainable)    # 52x256
        self.conv70 = self.conv_layer(self.conv69, 1, 1, 256, 128, True, 'conv_head_70', self.trainable)    # 52x128
        self.conv71 = self.conv_layer(self.conv70, 3, 1, 128, 256, True, 'conv_head_71', self.trainable)    # 52x256
        self.conv72 = self.conv_layer(self.conv71, 1, 1, 256, 128, True, 'conv_head_72', self.trainable)    # 52x128
        self.conv73 = self.conv_layer(self.conv72, 3, 1, 128, 256, True, 'conv_head_73', self.trainable)    # 52x256
